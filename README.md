# Tiempo — Tu Clima en Tiempo Real - Version 4.6.1
...
- **Pronostico Detallado:** Tiempo actual, prevision por horas (24h), a 7 días (colapsable: 4 días por defecto, expandible) y **gráfico de probabilidad de lluvia** con área/línea SVG para las próximas 24h.
...
- **Widgets de pantalla de inicio:** 4 widgets nativos Android adaptativos: tiempo actual (2×2), reloj+tiempo (4×1), previsión 5-7 días (4×2) y gráfica de lluvia en línea (4×2). Todos escalan fuentes y layout al tamaño real del widget. Redimensionables al 100% del ancho de la pantalla gracias a `maxResizeWidth: 1000dp`. Indicador `· hace Xh` cuando los datos tienen más de 1 hora. Soporte de tema claro/oscuro automático. Actualización en segundo plano cada 30 minutos.
...
- **Claves API configurables:** El usuario puede introducir su propia API Key de OpenWeatherMap y AEMET OpenData desde Ajustes. OWM desbloquea capas adicionales del mapa (temperatura, viento, presión); AEMET activa alertas oficiales y notificaciones costeras.
...
| **OpenWeatherMap** | Tiles de temperatura, viento, presion | Si (Ajustes app) |
